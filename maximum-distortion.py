...
EPSILON_VALUES = [0.01, 0.003, 0.005, 0.01, 0.03, 0.05, 0.1]
# EPSILON_VALUES = [0.01, 0.02, 0.05]
# zero for each epsion value
weight_distributions = []
weight_distributions.append(torch.tensor([x for x in range(args.num_classes)]))
# weight_distributions.append(get_weight_distribution(rankings, 7, 5))
# weight_distributions.append(get_weight_distribution(rankings, 7, 10))
# weight_distributions.append(get_weight_distribution(rankings, 7, 15))
# weight_distributions.append(get_weight_distribution(rankings, 7, 20))
flipped_labels = np.zeros((2, len(EPSILON_VALUES)))
...
for i, (tensor_batch, labels) in enumerate(data_loader):
    ...
    for epsilon_index, epsilon in enumerate(EPSILON_VALUES):
        ...
        if args.attack_type == 'PGD':
            adversarials0 = pgd(model, tensor_batch, target, eps=epsilon, device="cuda")
            adversarials1 = pgd(model, tensor_batch, target, threshold=args.th, eps=epsilon, device="cuda")
