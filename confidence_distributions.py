...
import numpy as np
from attacks import pgd, fgsm, mi_fgsm, get_weights, a_mi_fgsm
...
for i, (tensor_batch, labels) in enumerate(data_loader):
    ...
    # Do the inference
    with torch.no_grad():
        outputs = model(tensor_batch).detach().cpu().numpy()
        positive_outputs = list(outputs[outputs > 0])
        negative_outputs = list(outputs[outputs <= 0])
        positive_confidences.extend(positive_outputs)
        negative_confidences.extend(negative_outputs)
