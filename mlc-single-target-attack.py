...
for target_label_id, target_label in enumerate(TARGET_LABELS):
    ...
    for i, (tensor_batch, labels) in enumerate(data_loader):
        ...
        # If 1 or more samples were wrongly predicted to have target class, drop the whole batch
        if torch.sum(pred[:, target_label]).item() > 1:
            continue
...
plt.title("{0}, {1}".format(args.dataset_type, args.attack_type))
plt.legend()
plt.savefig('flip-up-curves-{0}'.format(args.dataset_type))
